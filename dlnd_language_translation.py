...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    source_text_to_ids = [[source_vocab_to_int[word] for word in line.split()] for line in source_text.split('\n')]
    target_text_to_ids = [[target_vocab_to_int[word] for word in line.split()] for line in target_text.split('\n')]
    eos = target_vocab_to_int['<EOS>']
    target_text_to_ids = [line + [eos] for line in target_text_to_ids]
    
    return source_text_to_ids, target_text_to_ids
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    end_id = tf.strided_slice(target_data, 
                              begin=[0, 0], 
                              end=[batch_size, -1], 
                              strides=[1, 1])
    
    decoding_input = tf.concat([tf.fill([batch_size, 1], target_vocab_to_int['<GO>']), end_id], 1)
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    """
    # TODO: Implement Function
    decoding_train_fn = tf.contrib.seq2seq.simple_decoder_fn_train(encoder_state)
    train_pred, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell, decoding_train_fn, 
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    """
    # TODO: Implement Function
    dec_infer_fn = tf.contrib.seq2seq.simple_decoder_fn_inference(output_fn, encoder_state, 
                                                                dec_embeddings, start_of_sequence_id, 
                                                                end_of_sequence_id, maximum_length, vocab_size)
    logits_infer, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell, dec_infer_fn, scope=decoding_scope)
    return logits_infer
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    """
    # TODO: Implement Function
    embed_inputs = tf.contrib.layers.embed_sequence(input_data, source_vocab_size, enc_embedding_size)
    enc_state = encoding_layer(embed_inputs, rnn_size, num_layers, keep_prob)
    ...
    training_logits, infer_logits = decoding_layer(dec_embed_input, dec_embeddings, enc_state, 
...
with tf.Session(graph=train_graph) as sess:
    ...
    saver.save(sess, os.path.join(os.getcwd(), save_path))
